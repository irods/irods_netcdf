...
extern "C" {
    ...
    **/
    ...
    msiH5Dataset_read_attribute (msParam_t *inpH5DatasetParam, msParam_t *outH5DatasetParam, ruleExecInfo_t *rei)
    {
        ...
        RE_TEST_MACRO ( ( char * )"    Calling msiH5Dataset_read_attribute")
        ...
        if (inpH5DatasetParam == NULL || outH5DatasetParam == NULL) {
            ...
            rodsLogAndErrorMsg (LOG_ERROR, &rsComm->rError, rei->status,
              ( char * )"msiH5Dataset_read_attribute: NULL input/output Param");
            return (rei->status);
        }
        ...
        if (strcmp (inpH5DatasetParam->type, h5Dataset_MS_T) == 0) {
            ...
            rodsLogAndErrorMsg (LOG_ERROR, &rsComm->rError, rei->status,
              ( char * )"msiH5Dataset_read_attribute: wrong inpH5DatasetParam type %s",
        ...
        if (l1descInx < 0) {
            ...
            rodsLogAndErrorMsg (LOG_ERROR, &rsComm->rError, rei->status,
              ( char * )"msiH5Dataset_read_attribute: bad fid %d", ind->fid);
            return (rei->status);
        }
    ...
    irods::ms_table_entry*  plugin_factory( ) {
        ...
        msvc->add_operation( "msiH5Dataset_read_attribute", "msiH5Dataset_read_attribute" );
